  ...
  return (
    <div className="px-6 py-8">
      <div className="mb-6 font-mono text-xs">
        ...
        <p className="text-[#808080] mt-1 text-sm">// Android Projects</p>
      </div>
      ...
      <div className="space-y-4">
        {projects.map((project) => (
          <div
            key={project.id}
            className="bg-[#313335] rounded border border-[#323232] overflow-hidden hover:border-[#3DDC84] transition-all"
          >
            {/* App Screenshot */}
            <div className="h-32 bg-gradient-to-br from-[#3DDC84] to-[#2BAF66] flex items-center justify-center relative overflow-hidden">
              {project.image ? (
                <img 
                  src={project.image} 
                  alt={project.title}
            ...
            {/* Project Details */}
            <div className="p-4">
              <h3 className="text-base font-bold text-[#A9B7C6] mb-2">
                {project.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {project.technologies.map((tech, idx) => (
                  <span
                    key={idx}
                    className="px-2 py-0.5 text-[10px] font-mono bg-[#2B2B2B] text-[#3DDC84] rounded border border-[#323232]"
                  >
                    {tech}
                  </span>
                ))}
              ...
              {/* Action Buttons */}
              <div className="pt-3 border-t border-[#323232]">
                <a
                  href={project.githubLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 px-4 py-2 bg-[#3DDC84] hover:bg-[#45E68E] text-[#2B2B2B] rounded transition-all text-xs font-semibold"
                >
